...
This provides a trigger when devices are connected or disconnected from the system.

The ~kind~ is ~device-connection~, and you can listen to various ~events~:

+ ~Add~ - Device was connected
+ ~Remove~ - Device was disconnected (not yet fully supported)

You specify the ~device_types~ using platform-agnostic identifiers:

+ ~usb~ - USB devices
+ ~storage~ - Storage devices (hard drives, USB drives, etc.)
+ ~network~ - Network interfaces
+ ~any~ - All device types (expands to usb, storage, and network)

You can specify multiple device types to monitor. For each type, a separate listener
will be registered.

Only ~macOS~ works with ~device-connection~ currently.
...
This listens to USB device connection events.

To monitor multiple device types:

#+begin_src toml
[[triggers]]
kind = "device-connection"
events = ["Add"]
device_types = ["usb", "storage"]
#+end_src

Or to monitor all device types:
...
When a device connection event occurs, the following state variables are available
for use in conditions and executors (after calling ~sytter-vars~):

+ ~sytter_device_name~ - Human-readable device name (e.g., "Apple Magic Keyboard")
+ ~sytter_device_vendor_id~ - Vendor ID in hex format (e.g., "0x05ac" for Apple)
+ ~sytter_device_product_id~ - Product ID in hex format
+ ~sytter_device_serial_number~ - Serial number (uniquely identifies THIS specific device, not just the model)
+ ~sytter_device_location_id~ - USB location ID (identifies which port the device is plugged into)
+ ~sytter_device_bsd_name~ - BSD device name for storage devices (e.g., "/dev/disk2")

**Important**: You must call ~sytter-vars~ at the start of your shell script to load
these variables as environment variables.

Example using device properties in a condition:
...
script = '''
sytter-vars
# Only trigger for Apple devices (vendor ID 0x05ac)
test "$sytter_device_vendor_id" = "0x05ac"
'''
#+end_src

Example showing all device info:
...
echo "Device: $sytter_device_name"
echo "Vendor ID: $sytter_device_vendor_id"
echo "Product ID: $sytter_device_product_id"
echo "Serial: $sytter_device_serial_number"
'''
#+end_src

**Discovering device properties**: You can use ~ioreg~ on macOS to discover what properties
are available for your devices:

#+begin_src sh
# List all USB devices with their properties
ioreg -r -c IOUSBHostDevice

# Find a specific device by name
ioreg -r -c IOUSBHostDevice -n "YourDeviceName"
...
If you need fine-grained control over the macOS implementation, you can use the
~macos_~ prefixed fields:

+ ~macos_device_class~ - Override the IOKit device class (e.g. ~IOUSBHostDevice~,
  ~IOMedia~, ~IONetworkInterface~)
+ ~macos_notification_type~ - Override notification timing: ~FirstMatch~ (fires
  when drivers start, default) or ~FirstPublish~ (fires when device registers,
  earlier)
...
events = ["Add"]
device_types = ["usb"]
macos_notification_type = "FirstPublish"
#+end_src

Or bypass the mapping entirely:

#+begin_src toml
[[triggers]]
kind = "device-connection"
events = ["Add"]
macos_device_class = "IOBluetoothDevice"
#+end_src
...
#+RESULTS:
: Babysit your system with IFTTT automation.
:
: Usage: sytter [OPTIONS]
:
: Options:
:   -s, --sytters-path <SYTTERS_PATH>
...
In your Nix module, assuming you provided the entirety of your Flake inputs as
~flake-inputs~:
#+begin_src nix :results none :exports code
{ flake-inputs, ... }: {
  imports = [
  ...
  services.sytter = {
    enable = true;
    # TODO: Show off some config!  But only after we've moved to YAML.
  };
}
#+end_src
...
Sytter provides standard health check endpoints following common conventions:

+ ~GET /health~ :: Standard REST API health check endpoint. Returns HTTP 200 OK
  with JSON body: ~{ "status": "ok", "version": "0.1.0" }~. This is a basic
  liveness check indicating the HTTP server is responding.

+ ~GET /healthz~ :: Kubernetes/Google convention health check endpoint (the "z"
  suffix avoids collisions with application routes). Returns the same response
  as ~/health~. Use this endpoint for Kubernetes liveness probes or other
  container orchestration systems.

Both endpoints return the same response and can be used interchangeably. The
~/health~ endpoint follows standard REST conventions, while ~/healthz~ provides
compatibility with Kubernetes and other container orchestration platforms.
