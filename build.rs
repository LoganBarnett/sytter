...
fn macos_binding_generate() -> Result<(), BuildError> {
    ...
    let sdk_path = String::from_utf8(output.stdout)
        ...
        // Rust idiom error when using trim() here.  Otherwise we have to create
        // an entirely separate variable, like an animal.
        .to_owned();
    let framework_path = format!("-F{}/System/Library/Frameworks", sdk_path);
    println!("bindgen:isysroot={sdk_path}");
    // Tell cargo to look for shared libraries in the specified directory
    println!("cargo:rustc-link-search={framework_path}");
    ...
    let bindings_res = bindgen::Builder::default()
        // time.h as has a variable called timezone that conflicts with some of
        // the objective-c calls from NSCalendar.h in the Foundation framework.
        // This removes that one variable.
        ...
        // *c_void, which the function actually expects.  Without this we run
        // into problems where Rust is using some kind of wrapped value.  We
        // can't pass the original value to the function and so we never
        // actually do the allow.  See bindings.rs for the manual declaration.
    ...
    match bindings_res {
        Ok(bindings) => {
            ...
            // control (or an easily readable location).
            if false {
                ...
                // This gets a little more complex than using
                // Bindings.write_to_file because we need a prefix the file data
                // to prevent rustfmt from playing tug-of-war with the bindings.
                // The code is generated and so we shouldn't fuss too much about
                // the formatting, _or_ we should format the code _after_ it's
                // generated.  Perhaps in the future this could be done
                // programmatically to the buffer prior to writing it to the
                // file.
        ...
        Err(e) => {
            // We can continue because we've already written the bindings
            // earlier.  The bindings should not be a breaker to building.
            println!("Got error {:?} but continuing anyways.", e);
        }
    };
    Ok(())
}
